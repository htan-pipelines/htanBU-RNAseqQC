...
params:
  ...
  # `collection_site`, `cohort`, and `batch_id` are all fully optional -- if your
  # cohort doesn't have one of these concepts (e.g. a single-tissue study, or no
  # tracked sequencing batch), just omit that entry (or leave it NULL) and the
  # corresponding sections/plots are skipped automatically.
...
# `batch_id` is fully optional: cohorts that don't track a sequencing batch simply
# skip the "Batch information" / PCA-by-batch section below.
has_batch_id <- "batch_id" %in% colnames(rnaAnnot)
...
# Preprocess the genotypes param (already read in by render_qc_report.R 
genotypes <- genotypes %>%
  arrange(subject) %>%
...
```{r batch_header, results = "asis"}
# results='asis' + a conditional cat() heading (same pattern as tissue_type_panel
# above) so the whole "Batch information" tabset is skipped ENTIRELY -- not just
# left empty -- when there's no batch_id column. A plain `eval=` on the plot chunk
# alone would still leave behind these static markdown headings with nothing under
# them, since they live outside the chunk.
if (has_batch_id) {
  cat("\n\n## Batch information {.tabset .tabset-fade}\n\n")
  cat("### PCA batch\n\n")
  cat("color PCA plot by batch and see if you observe visual separation of the samples\n\n")
}
```

```{r, pca_batch, fig.width = 10, fig.height = 8, eval = has_batch_id}
